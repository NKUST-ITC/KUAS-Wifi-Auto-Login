...
public class WidgetService extends Service {
    ...
    private void changeWidgetMessage(String message, int color) {
        ...
        ComponentName thisAppWidget = new ComponentName(getPackageName(), WidgetProvider.class.getName());
    ...
    public void login() {
        ...
        LoginHelper.login(this, Utils.tranUser(user, pwd), new GeneralCallback() {
            ...
            public void onAlready() {
                mTracker.send(new HitBuilders.EventBuilder().setCategory("login").setAction("alreadyLogin").build());
                changeWidgetMessage(getResources().getString(R.string.widget_already_connect), getResources().getColor(R.color.md_green_400));
                stopSelf();
            }
        });
    }
